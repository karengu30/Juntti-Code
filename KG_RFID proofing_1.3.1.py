#after two months, this code works!
#at some point, it appears to multiply the occurence of every datetime RFID reading by 4
    #this is easily correctable by dividing value counts by 4 before appending them to freqlist, but why is it happening?
import pandas as pd
import numpy as np
...
"""
input_file = '/Users/johnmerlo-coyne/Desktop/Juntti Lab Stuff/RFID Project/RFID Output Sheets/10.23.20_karen.xlsx' #paste filepath here, including file name- end with .xlsx
target_fishid = '3D6.1D59B07986' #from spreadsheet, paste fishid name here in quotes

start_date, end_date = str('09/25/2020'), str('09/26/2020')
...
df = pd.read_excel(input_file) #import RFID output sheet from filepath
...
print('TESTING SPREADSHEETS AGAINST EACH OTHER!')
print(df.head(20))
...
fig.tight_layout()
title_name = ('RFID frequency for fish ' + target_fishid + ": " + str(start_date) + " to " + str(end_date))
#^puts together graph title, using name of fish in question, and target dates
plt.title(title_name)
plt.xlabel('Date and Time')
plt.ylabel('Number of RFID Readings')
