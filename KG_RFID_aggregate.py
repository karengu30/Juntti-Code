...
class FishAggregator: #create class
    ...
    def aggregate_two(self, twofish): #combines RFID readings for TWO fishids of your choice, between dates of your choice, into one dataframe-
        #sets this dataframe as self.fishids_aggregated_by_date, callable in other methods
        self.fishid_fishdict = {} #create empty dictionary to hold spreadsheets for all target fishids
        for i in twofish: #for each target fishid...
            pathname = self.filepath_out + i + '_alldates.xlsx' #combines name and filepath to test if there is already a sheet for the fishid
        #print('CHECKING FOR EXISTING FILENAMES: ' + pathname + '...')
            if path.exists(pathname): #if there is already a spreadsheet with that filename...
                previous_file = pd.read_excel(pathname) #read in that spreadsheet as a dataframe
        ...
        output_dict = {}
        for i in twofish: #for each target fishid...
            #df_for_graphing = pd.DataFrame()
            fishid_fishlist = self.fishid_fishdict[i] #pull out the dataframe in the dictionary of dataframes for that fishid
            culled_fishlist = fishid_fishlist[(fishid_fishlist['ScanDate'].apply(self.convert_date) >= target_start) & (fishid_fishlist['ScanDate'].apply(self.convert_date) < target_end)].copy()
            #^for that fishid dataframe, pulls out only the RFID readings for the daterange between target_start and target_end
            #df_for_graphing = df_for_graphing.append(culled_fishlist) #add it to the dataframe of all fishids for that date range
            output_dict[i] = culled_fishlist
        #self.fishids_aggregated_by_date = output_dict #self.fishids_aggregated_by_date can now be used in different methods within the class
        print('GENERATED COMBINED DATAFRAME FOR FISHIDS:' + str(twofish) + ' BETWEEN DATES: ' + self.target_start + ' - ' + self.target_end) #prints that dataframe for fishids and dates has been created
        return(output_dict)
    ...
    def within(self, fishdict, sec_within, fem, male): #input dictionary with 2 fishids in it
        fishdict[fem] = self.convert_to_datetime(fishdict[fem])
        #rff = fishdict[fem]
        fishdict[male] = self.convert_to_datetime(fishdict[male])
        #rfm = fishdict[male]
        fishdict[fem]["within"] = ""
        index_col_within = fishdict[fem].columns.get_loc("within")
        index_col_mal_SDT = fishdict[male].columns.get_loc('ScanDateTime')
        index_col_fem_SDT = fishdict[fem].columns.get_loc('ScanDateTime')
        pointer = 0
        for i in range(len(fishdict[fem]['ScanDateTime'])):
            min = fishdict[fem].iloc[i,index_col_fem_SDT] - timedelta(seconds = sec_within)
            max = fishdict[fem].iloc[i,index_col_fem_SDT] + timedelta(seconds = sec_within)
            for p in range(pointer, len(fishdict[male]['ScanDateTime'])):
                if min <= fishdict[male].iloc[p,index_col_mal_SDT] <= max:
                    fishdict[fem].iloc[i,index_col_within] = 1
                    pointer = p
                    break
                elif fishdict[male].iloc[p,index_col_mal_SDT] >= max:
                    fishdict[fem].iloc[i,index_col_within] = 0
                    pointer = p
                    break
        fishdict[fem] = fishdict[fem][fishdict[fem].within == 1]
        rff = fishdict[fem].copy()
        return(rff) #return a single dataframe containing

    def within2(self, fishdict, sec_within, fem, male): #input dictionary with 2 fishids in it
        fishdict[fem] = self.convert_to_datetime(fishdict[fem])
        rff = fishdict[fem].copy()
        #rff = fishdict[fem]
        fishdict[male] = self.convert_to_datetime(fishdict[male])
        #rfm = fishdict[male]
        rff["within"] = ""
        index_col_within = rff.columns.get_loc("within")
        index_col_mal_SDT = fishdict[male].columns.get_loc('ScanDateTime')
        index_col_fem_SDT = rff.columns.get_loc('ScanDateTime')
        pointer = 0
        for i in range(len(fishdict[fem]['ScanDateTime'])):
            min = rff.iloc[i,index_col_fem_SDT] - timedelta(seconds = sec_within)
            max = rff.iloc[i,index_col_fem_SDT] + timedelta(seconds = sec_within)
            for p in range(pointer, len(fishdict[male]['ScanDateTime'])):
                if min <= fishdict[male].iloc[p,index_col_mal_SDT] <= max:
                    rff.iloc[i,index_col_within] = 1
                    pointer = p
                    break
                elif fishdict[male].iloc[p,index_col_mal_SDT] >= max:
                    rff.iloc[i,index_col_within] = 0
                    pointer = p
                    break
        rff = rff[rff.within == 1]

        return(rff) #return a single dataframe containing

    def create_withinBargraph(self, rff, outputdict, fem, hour_start, hour_end): #from a dataframe of RFID readings for a group of target fishids between a target daterange, produces a bargraph of RFID counts binned by hour
        print('CREATING BARGRAPH FOR FISHIDS: ' + str(self.target_fishids) + ' FOR TARGET DATES: ' + target_start + ' - ' + target_end + '...')
        #df = self.fishids_aggregated_by_date #reads in the aggregated df of target fishids in the target daterange
        rff = self.convert_to_datetime(rff) #creates a column of datetime objects for the RFID reading (df['ScanDateTime'])
        rff['HEXTagID'] = rff['HEXTagID'] + '_within'
        rff['ScanDateTimeShort'] = rff['ScanDateTime'].values.astype('datetime64[h]') #cuts the datetime column to just month, day and hour
        possible = pd.date_range(self.target_start, self.target_end, freq='h') #creates list of all datetimes between start dayhour and end dayhour
        possible = possible[:-1] #removes the first hour of end date (output will not include any hour in end date)
        possible_culled = [] #create empty list to hold possible all hours (including those with 0 entries) that will be in bar graph
        for i in possible: #adds hours between 08:00 and 20:00 to a new series, then makes that series into 'possible'
        ...
        df_tograph = pd.DataFrame(possible_culled, columns= ['possible_culled']) #create empty dataframe- first column is list of all possible datehours to be graphed, column name is possible_culled
                                                #^will contain columns with the RFID frequency for each datehour in possible_culled for each target fishid
        df_target_fishid = outputdict[fem]
        #df_target_fishid = df[df['HEXTagID'] == fem]
        df_tograph[fem] = self.get_fishid_occurences_for_datehour_range(df_target_fishid, possible_culled)
        df_tograph[fem+'_within'] = self.get_fishid_occurences_for_datehour_range(rff, possible_culled)

        fig, ax = plt.subplots(figsize=(18,7))
        df_tograph.plot(x='possible_culled', y=[fem, fem+'_within'], kind='bar', width= self.barwidth, ax=ax)
        #fig.autofmt_xdate()
        ax.set_xticklabels(labels= df_tograph.possible_culled, rotation=55, ha='right')
        ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=1))
        #ax.set_ylim([0,500])
        #ax.set_xlim([datetime.strptime(self.target_start, '%m/%d/%Y'), datetime.strptime(self.target_end, '%m/%d/%Y')])
        plt.title('RFID readings for fishids: ' + str([fem, fem+'_within']) + ' between dates: ' + self.target_start + ' - ' + self.target_end) #creates title for figure, noting fishid and dates
        plt.xlabel('Date and Time (H)')
        plt.ylabel('RFID Readings')
        plt.tight_layout()
        plt.subplots_adjust(left= 0.05)
    ...
    def get_fishid_occurences_for_datehour_range(self, fishid_df, possible_datehours): #given a df of RFID readings for only one target fishid, and a list of datehours to be graphed, returns a list of that fishid's RFID counts for each datehour to be graphed
        ...
        for i in possible_datehours: #for each datehour in the list of all datehours between the start and end dates and between 08:00 and 20:00...
            if ((i.year==fishid_df['ScanDateTime'].dt.year) & (i.month==fishid_df['ScanDateTime'].dt.month) & (i.day==fishid_df['ScanDateTime'].dt.day) & (i.hour==fishid_df['ScanDateTime'].dt.hour)).any(): #if the df for the input fishid contains that datehour...
                boolarr = (i.year==fishid_df['ScanDateTime'].dt.year) & (i.month==fishid_df['ScanDateTime'].dt.month) & (i.day==fishid_df['ScanDateTime'].dt.day) & (i.hour==fishid_df['ScanDateTime'].dt.hour)
                freqlist.append(boolarr.sum()) #return the number of RFID readings for that datehour
        ...
        #TODO: IS THE IF ELSE STATEMENT REALLY NECESSARY?

# agtwo=fishobject.aggregate_two(['3D6.1D59B07940','3D6.1D59B07932'])
# rff_within = fishobject.within2(agtwo, 2, '3D6.1D59B07940', '3D6.1D59B07932')
# fishobject.barwidth = 1
# fishobject.create_withinBargraph(rff_within, agtwo, '3D6.1D59B07940', 8, 20)
...
target_start, target_end = '11/08/2021', '11/10/2021'  #for graphing, etc- what dates should the program act on (plotting)
